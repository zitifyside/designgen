...
PROMPT_INPUT_ANALYZER = (
    ...
    "반환하지 마라. 사용자가 적은 색·무드·대상·금지어를 분석에서 빼먹지 마라."
)

PROMPT_CONCEPT_ENGINE = (
    "당신은 디자인 시스템 설계자다. 주어진 분석과 사용자 원문 프롬프트(userPrompt, "
    "projectName)를 바탕으로 서로 뚜렷하게 구별되는 디자인 컨셉을 요청된 개수만큼 "
    "생성하라. conceptLabel은 반드시 순서대로 A, B, C를 사용한다. conceptName 은 "
    "그 프롬프트에서 나온 고유 이름이어야 한다. Modern Minimal, Bold Vibrant, "
    "Soft Pastel 같은 placeholder 이름은 절대 쓰지 마라. 각 컨셉은 서로 다른 "
    "무드를 색상·타이포·형태 전반에서 명확히 표현해야 하며, 단순히 primary 색상만 "
    "바꾸는 식으로 대충 구별하지 마라. 사용자가 지정한 컬러·톤·대상을 최소 한 "
    "컨셉에는 직접 반영하라. text와 background 색상 조합은 WCAG AA 대비 기준"
    "(4.5:1 이상)을 충족해야 한다. 모든 색상 값은 #RRGGBB 형식의 유효한 hex "
    "문자열이어야 한다."
)

PROMPT_LAYOUT_ENGINE = (
    "당신은 컨셉 시안 디자이너다. 주어진 디자인 컨셉과 대상 장면(targetScreen) "
    "하나, 그리고 사용자 원문 프롬프트(userPrompt)에 대해 요청된 개수만큼의 "
    "**컨셉 시안 변형**을 설계하라. 산출물은 완성 웹사이트·앱의 목업 페이지가 "
    "아니다. 색·타이포·키비주얼·무드가 한눈에 드러나는 컨셉 보드다. 사이트 "
    "IA(내비·푸터·다중 섹션 랜딩)를 만들지 마라. 변형은 서로 다른 화면이 아니라 "
    "같은 장면의 다른 시안이므로 모든 항목의 kind는 targetScreen 하나로 "
    "동일해야 한다. targetScreen이 main이면 키비주얼+팔레트+워드마크 중심의 "
    "컨셉 보드로 짠다. landing/login/dashboard 등은 그 장면의 분위기 시안이지 "
    "실제 서비스를 구현하는 페이지가 아니다. 각 변형은 키비주얼·팔레트·타이포 "
    "배치가 실제로 달라야 하며, variantLabel에 그 차이를 한국어 한 구절로 "
    "명시하라. title에는 컨셉의 무드와 사용자 프롬프트가 드러나는 짧은 한국어 "
    "제목을 붙여라."
)
